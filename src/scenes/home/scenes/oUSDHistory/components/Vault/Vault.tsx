...
import { getLendPool, getVaultHistory, lendHistory } from "service/http/http";
...
> {
  ...
  async getResult() {
    ...
    //   .add(1, "day")
    //   .format("YYYY-MM-DD HH:mm:ss");
    const result = await getVaultHistory(param);
    // const time: number =
    //   result.success &&
      ...
      <Loadding show={this.state.loading}>
        <Row className="Vault">
          <Table
            ...
            dataSource={this.state.result.list}
            scroll={{ x: true }}
            locale={{
...
export default connect(mapStateToProps, mapDispatchToProps)(injectIntl(Vault));
